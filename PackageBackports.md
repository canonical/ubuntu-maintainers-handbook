...
their *security updates process*. And, teams individuals may also use
...
individual, well-understood defects. Backport SRUs, in contrast,
...
* **Policy Adherence:** Always refer to the
  ...
  As well, the software being backported should also have an
  [SRU Package-Specific Exception](https://documentation.ubuntu.com/sru/en/latest/reference/package-specific/).

* **Testing:** Comprehensive testing is non-negotiable. This includes
  building the package on target architectures and performing testing as
  defined by the package’s SRU exception.

* **Clear Justification:** For the Backport SRU bug reports, clearly
  articulate the rationale for the backport with a link to the
  package-specific exception page, what notable changes are in the
  backport, how it was tested, and any irregularities encountered during
  packaging or testing. Crucially, highly any changes or findings that
  might be of concern for regression risk.
...
* **Signed GPG Key:** For signing commits and package builds.

* **Launchpad Account:** With necessary permissions for contributing to
  Ubuntu packages.

(Refer to the "Getting Started" section of the
...
2. **Update the Bug Report:**
   ...
   report, then file one yourself for tracking purposes.
   ```
   lp\_bug\_number=2085197
   lp-affects-devel ${lp\_bug\_number} ${package} ${codename}
   ```

3. Initialize the Job Directory:
   Create a dedicated directory for your backport work.
   ```
   mkdir \-p \~/pkg/${package}/backport-lp${lp\_bug\_number}/
   cd \~/pkg/${package}/backport-lp${lp\_bug\_number}/
   ```
...
2. **Adapt debian/changelog:** The changelog will now reflect the
   history of the newer release. You must add a new entry to the *top*
   of the debian/changelog file for the backport.

   * Change the distribution to your target (e.g., noble).
   * Increment the Ubuntu revision (e.g., 1.2.3-1ubuntu0.24.04.1).
   * Add a clear description indicating it's a backport, the source release, and why.

package-name (1.2.3-1ubuntu0.24.04.1) noble; urgency=medium

  ```
  * Backport 1.2.3-1 from Ubuntu questing (LP: NNNNNNN)
    - Includes new upstream release 1.2.3.
    - Incorporates updated questing packaging.

  -- Your Name <your.email@example.com>  <Date and Time>

3. **Review debian/control and Other Packaging Files:**
...
4. **Commit Changes:** In this style of backport, you will generally
   have a single commit for the backport itself that represents all the
   changes (tarball, packaging, and changelog) from the original Ubuntu
   release, and subsequent separate commits for each discrete packaging
   change (if any).

5. **Build and Test the Package:** Build the package and perform
...
1. **Obtain New Upstream Tarball:** Identify the new upstream tarball (e.g., foo\_1.2.3.orig.tar.gz).
...
2. Checkout the Target Release and Create New Branch:
   ...
   git checkout --no-track pkg/ubuntu/${codename}-devel -b ${branch\_name}
   ```
...
8. **Adapt Packaging to New Upstream:**
   ...
   * **debian/changelog:** Add a new entry indicating the upstream
     ...
     ```
     package-name (new-upstream-version-XubuntuY.Z.backport1) noble; urgency=medium
...
test cases documented in 7 bug reports.
...
3. **Create a New Branch:** Always work on a new branch for your
   ...
   * **Apply patch:** Resolve each patch carefully, ensuring the fix is
     correctly applied without introducing regressions. Strive to keep
...
4. **Update debian/changelog:** Add an entry to debian/changelog using
   dch \-i (or dch \--bpo), clearly describing the backported fix,
   referencing the Launchpad bug, and noting that it's a
   backport. Increment the Ubuntu revision number appropriately.

   ```
   package-name (version-0ubuntu24.04.1) noble; urgency=medium
...
as defined in the
[Ubuntu Maintainer's Handbook](https://github.com/canonical/ubuntu-maintainers-handbook/blob/main/PackageBuilding.md).
...
Prepare for PPA build:
  ...
  install-build-deps
  ```
  4. Build the source package, specifying the version currently in the
     target release.
  ```
  ...
  changes_file="../${package}\_${new\_version}\~${codename}1\_source.changes"
...
* **Verification:** Perform testing documented by the SRU Package-Specific Exception.
...
4. **Request Sponsorship:** Once the MP is approved, request sponsorship
   from an Ubuntu Developer or MOTU if necessary. They will review your
   code, packaging, and testing before uploading to the archive.
